...
void HunterROSMessenger::SetupSubscription() {
  ...
  integrator_reset_subscriber_ = nh_->subscribe<std_msgs::Bool>("/reset_odom_integrator", 5, &HunterROSMessenger::ResetOdomIntegratorCallback, this);
   livox_imu_ = nh_->subscribe<sensor_msgs::Imu>("/Livox/imu", 5, &HunterROSMessenger::Livox_Imucb, this);
}
...
void HunterROSMessenger::SetupSubscription_multi(std::string car_name) {
  ...
  // cmd subscriber
  motion_cmd_subscriber_ = nh_->subscribe<geometry_msgs::Twist>(
      "/"+car_name+"/cmd_vel", 5, &HunterROSMessenger::TwistCmdCallback, this);
  integrator_reset_subscriber_ = nh_->subscribe<std_msgs::Bool>("/reset_odom_integrator", 5, &HunterROSMessenger::ResetOdomIntegratorCallback, this);
   livox_imu_ = nh_->subscribe<sensor_msgs::Imu>("/"+car_name+"/Livox/imu", 5, &HunterROSMessenger::Livox_Imucb, this);
}
...
void HunterROSMessenger::Livox_Imucb(const sensor_msgs::Imu::ConstPtr &msg)
{
	 livox_imu_x = msg->angular_velocity.x;
	 livox_imu_z = msg->angular_velocity.z;
}
